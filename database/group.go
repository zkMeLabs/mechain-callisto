...
func (db *DB) UpdateGroupToSQL(ctx context.Context, group *models.Group) (string, []interface{}) {
	stat := db.G.Session(&gorm.Session{DryRun: true}).Table((&models.Group{}).TableName()).Where("account_id = ? AND group_id = ? ", group.AccountAddress, group.GroupID).Updates(group).Statement
	return stat.SQL.String(), stat.Vars
}

func (db *DB) UpdateGroupByOwnerAndNameToSQL(ctx context.Context, group *models.Group) (string, []interface{}) {
	stat := db.G.Session(&gorm.Session{DryRun: true}).Table((&models.Group{}).TableName()).Where("account_id = ? AND owner = ? AND group_name = ?  AND removed=false", group.AccountAddress, group.OwnerAddress, group.GroupName).Updates(group).Statement
	return stat.SQL.String(), stat.Vars
}
...
func (db *DB) BatchDeleteGroupMemberToSQL(ctx context.Context, group *models.Group, groupID string, accountIDs []string) (string, []interface{}) {
	stat := db.G.Session(&gorm.Session{DryRun: true}).Table((&models.Group{}).TableName()).Where("account_id IN ? AND group_id = ? ", accountIDs, groupID).UpdateColumns(group).Statement
	return stat.SQL.String(), stat.Vars
}
