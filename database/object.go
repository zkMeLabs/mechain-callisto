...
func (db *DB) GetObject(ctx context.Context, objectId string) (*models.Object, error) {
	...
	if err := db.G.WithContext(ctx).Table((&models.Bucket{}).TableName()).Where(
...
func (db *DB) UpdateObjectToSQL(ctx context.Context, object *models.Object) (string, []interface{}) {
	stat := db.G.Session(&gorm.Session{DryRun: true}).Table((&models.Bucket{}).TableName()).Where("object_id = ?", object.ObjectID).Updates(object).Statement
	return stat.SQL.String(), stat.Vars
}

func (db *DB) UpdateObjectByBucketNameAndObjectNameToSQL(ctx context.Context, object *models.Object) (string, []interface{}) {
	stat := db.G.Session(&gorm.Session{DryRun: true}).Table((&models.Bucket{}).TableName()).Where("bucket_name = ? AND object_name =? AND removed=false", object.BucketName, object.ObjectName).Updates(object).Statement
	return stat.SQL.String(), stat.Vars
}
